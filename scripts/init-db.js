...
function run() {
  ...
  const upsert = db.prepare(`INSERT INTO global_state(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value`);
  ...
  // One-click setup: if there are no cases yet, seed a small demo catalog
  // so the site isn't blank on first deploy.
  const n = db.prepare('SELECT COUNT(*) as n FROM cases').get().n;
  if (n === 0) {
    console.log('No cases found — running demo seed...');
    require('./seed');
  }
}
